  ...
  if (Array.isArray(configFile)) {
    ...
    web.users.list({}, function(err, response) {
      ...
      response.members.forEach(function(item) {
        // Don't make IRC bots for known bots that exist on Slack or deleted users
        if (!item.deleted && item.name != 'slackbot' && item.name != 'irc-bridge') {
          setTimeout(function() {
            configFile.nickname = item.name + '-slack';
          ...
          // Delay in seconds between creating bots to prevent IRC join/quit spam
          i += 1;
        }
      });
    });
