...
const REQUIRED_FIELDS = ['server', 'nickname', 'channelMapping', 'token', 'slackUser'];
...
class Bot {
  constructor(options) {
    ...
    // Don't send duplicate messages to Slack! (Default false)
    this.canSendToSlack = options.canSendToSlack || false;
    this.ignoreFrom = [];

    // Record a user's Slack UID for message connection
  ...
  attachListeners() {
    ...
    this.slack.rtm.on('message', message => {
      ...
      // Only send the message if this is the correct IRC user
      if (message.type === 'message' &&
        (!message.subtype || ALLOWED_SUBTYPES.indexOf(message.subtype) > -1) &&
        message.user == this.slackUser) {
        this.sendToIRC(message);
      }

      // Leave messages are type 'message' and subtype 'channel_leave',
      // join messages are type 'message' and subtype 'channel_join'
    });

    // Only send messages to slack if told to do so
    if (this.canSendToSlack) {
  ...
  sendToSlack(author, channel, text) {
    ...
    this.ignoreFrom.forEach(function(item) {
      if (author.startsWith(item.nickname)) {
        logger.debug("Skipping message, is from a bot");
        skip = true;
        return;
      }
    });
    ...
    if (slackChannelName) {
      ...
      if (author !== this.nickname && this.avatarUrl) {
        ...
        iconUrl = this.avatarUrl.replace(/\$email_hash/g, crypto.createHash('md5').update(author + "@ocf.berkeley.edu").digest('hex'));
      }
